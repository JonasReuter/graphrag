...
async def test_entity_specs_returns_top_k_titles():
    ...
    store.similarity_search_by_vector.return_value = [
        _make_search_result("JOCHEN KRESS"),
        _make_search_result("TILL LINDNER"),
    ]
    model = _make_emb_model()

    result = await _entity_specs_per_chunk(
        texts=["chunk about Jochen Kress"],
        embedding_model=model,
        vector_store=store,
        top_k=5,
        fallback=["person"],
    ...
    assert result == [["JOCHEN KRESS", "TILL LINDNER"]]
...
async def test_entity_specs_filters_stale_titles_via_known_titles():
    store = MagicMock()
    store.count.return_value = 10
    # Index has pre-resolution title "TILL LINDNER" but canonical is "HERR LINDNER"
    store.similarity_search_by_vector.return_value = [
        _make_search_result("TILL LINDNER"),   # stale — not in known_titles
        _make_search_result("HERR LINDNER"),   # canonical ✓
    ]
    model = _make_emb_model()
    known = {"HERR LINDNER", "JOCHEN KRESS"}
...
async def test_entity_specs_falls_back_when_all_filtered():
    ...
    result = await _entity_specs_per_chunk(
        ...
        top_k=3,
        fallback=["person", "org"],
        known_titles={"JOCHEN KRESS"},  # STALE ENTITY not in known
    )
