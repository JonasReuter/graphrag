...
def _make_covariates(**overrides) -> pd.DataFrame:
    base = {
        "id": ["c1", "c2"],
        "subject_id": ["CUSTOMER A", "ME-MESSYSTEME GMBH"],
...
async def test_high_score_auto_resolved():
    ...
    # unique ids sorted alphabetically: "CUSTOMER A", "K6D40", "ME-MESSYSTEME GMBH"
    store = _make_vector_store(results_by_query=[
        [_make_search_result("CUSTOMER A", 0.97)],        # CUSTOMER A → high → auto
    ...
    assert result_df["subject_id"].tolist() == ["CUSTOMER A", "ME-MESSSYSTEME GMBH"]
    assert resolution_map["CUSTOMER A"] == "CUSTOMER A"
...
async def test_mid_score_llm_confirmed():
    ...
    df = _make_covariates(
        subject_id=["MR ALPHA", "MR ALPHA"],
        object_id=[None, None],
    )
    embedding_model = _make_embedding_model([[0.1] * 4])
    store = _make_vector_store(results_by_query=[
        [_make_search_result("AGENT ALPHA", 0.88)],
    ])
    completion_model = _make_completion_model(match=True)
    ...
    assert all(result_df["subject_id"] == "AGENT ALPHA")
...
async def test_low_score_unchanged():
    ...
    df = _make_covariates(subject_id=["UNKNOWN ENTITY", "UNKNOWN ENTITY"], object_id=[None, None])
    embedding_model = _make_embedding_model([[0.1] * 4])
    store = _make_vector_store(results_by_query=[
        [_make_search_result("MR ALPHA", 0.60)],
    ])
    completion_model = _make_completion_model(match=True)
    ...
    assert all(result_df["subject_id"] == "UNKNOWN ENTITY")
    completion_model.completion_async.assert_not_called()
